...
import { FiDatabase, FiPlus, FiServer, FiTrash2, FiRefreshCw, FiEdit3, FiArrowRight, FiFileText } from 'react-icons/fi';
...
import { apiService, HohAddressDatabase } from '../../services/api';
import { useAuth } from '../../contexts/AuthContext';
import { useTab } from '../../contexts/TabContext';
import './HohAddressTab.css';

const HohAddressTab: React.FC = () => {
  const { user } = useAuth();
  const { addTab } = useTab();
  const isAdmin = user?.role === 'admin';
  const [databases, setDatabases] = useState<HohAddressDatabase[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [databaseToEdit, setDatabaseToEdit] = useState<HohAddressDatabase | null>(null);
  ...
  const handleDatabaseAdded = () => {
    loadDatabases();
    // Notify sidebar to refresh HohAddress databases list
    window.dispatchEvent(new CustomEvent('hohaddress-databases-updated'));
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiService.deleteHohAddressDatabase(databaseToDelete.id);
      loadDatabases();
      // Notify sidebar to refresh HohAddress databases list
      window.dispatchEvent(new CustomEvent('hohaddress-databases-updated'));
    } catch (err) {
  ...
  const handleEnterDatabase = async (db: HohAddressDatabase) => {
  ...
  const handleOpenLogs = async (db: HohAddressDatabase) => {
  ...
  const handleEdit = async (db: HohAddressDatabase) => {
    // Load full database info
    const fullDb = await apiService.getHohAddressDatabase(db.id);
  ...
  const headerContent = (
    <div className="hohaddress-header">
      <div className="connections-title-wrapper">
        <FiEdit3 size={28} className="connections-title-icon" />
        <h1 className="connections-title">HohAddress</h1>
      </div>
      <div className="hohaddress-header-actions">
  ...
  return (
    <>
      <TabLayout header={headerContent} footer={footerContent}>
        ...
        {loading && (!databases || databases.length === 0) ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-description">Click "Add Database" to start working with HohAddress</p>
          </div>
        ) : (
          <div className="hohaddress-content">
      ...
      <ConfirmModal
        ...
        title="Remove Database"
        message={`Are you sure you want to remove "${databaseToDelete?.name}" from HohAddress?`}
        confirmText="Remove"
        cancelText="Cancel"
