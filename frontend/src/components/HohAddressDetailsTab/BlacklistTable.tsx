...
const BlacklistTable: React.FC<BlacklistTableProps> = ({ hohAddressDatabase }) => {
  ...
  const loadData = useCallback(
    async (customWhere?: string, customSearch?: string) => {
      try {
        ...
        console.log('Loading blacklist with WHERE:', whereClause, 'and search:', searchValue);

        const result = await apiService.getHohAddressBlacklist(
          hohAddressDatabase.id,
          filters,
          sortBy || undefined,
          sortOrder,
  ...
  const handleCreate = async (formData: Record<string, any>) => {
    try {
      await apiService.createHohAddressBlacklistRow(hohAddressDatabase.id, formData);
  ...
  const handleUpdate = async (rowId: string, formData: Record<string, any>) => {
    try {
      await apiService.updateHohAddressBlacklistRow(hohAddressDatabase.id, rowId, formData);
      setEditingRow(null);
      loadData();
    } catch (err: any) {
  ...
  const handleDelete = async (rowId: string) => {
    ...
    try {
      await apiService.deleteHohAddressBlacklistRow(hohAddressDatabase.id, rowId);
      loadData();
    } catch (err: any) {
