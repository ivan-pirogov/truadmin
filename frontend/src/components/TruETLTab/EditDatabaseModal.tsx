...
import { apiService, Connection, Database, TruETLDatabase } from '../../services/api';
import './AddDatabaseModal.css';

interface EditDatabaseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: () => void;
  database: TruETLDatabase | null;
}
...
const EditDatabaseModal: React.FC<EditDatabaseModalProps> = ({ isOpen, onClose, onUpdate, database }) => {
  ...
  const loadEligibleDatabases = async (connectionId: string) => {
    try {
      ...
      setError(null);
      const data = await apiService.getEligibleDatabases(connectionId);
      const databases = data || [];
      setDatabases(databases);
      if (databases.length === 0) {
        setError('No eligible databases found (databases must have meta.dms_tables table)');
      }
    } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      await apiService.updateTruETLDatabase(database.id, selectedConnection, selectedDatabase, displayName || undefined);
      onUpdate();
      handleClose();
    } catch (err: any) {
                ...
                <>
                  ...
                  {databases.length > 0 && (
                    <span className="form-hint">
                      Only databases with meta.dms_tables table are shown
                    </span>
                  )}
                </>
              )}
